...
(2) `어시스턴트`앱을 엽니다. 
 . 브라우저 우측 상단에 앱 목록 버튼 클릭
 . 목록에서 `네이버 전환  스크립트 어시스턴트`앱의 Pin 아이콘 클릭
 . 우측 상단에 나온 N아이콘 클릭
 . `어시스턴트` 앱이 열림
...
![06_14_manual_example_02.png]({{"/assets/img/06_14_manual_example_02.png"| relative_url}})

※ 참고
혹시 광고 클릭 후 창이 새로 열린다면, 새로 열린 브라우저 화면에서 우측상단 `N아이콘` 을 클릭하여 `어시스턴트`앱을 열어주세요
앱을 열면, 앱을 열기 전, 발생한 해당 창이 열릴 때 발생했던 로그들과 관련 정보들을 확인하실 수 있습니다. 

![06_14_manual_example_03.png]({{"/assets/img/06_14_manual_example_03.png"| relative_url}})
...
(4-1) 정상적인 경우 `어시스턴트`앱에 다음과 같은 정보가 보이게 됩니다.
 ...
 ▶ 최상단 부분
 . 사이트ID '1개', 로그 '1건' 혹은 '2건', 네이버 광고 쿠키 '3건' 혹은 '4건', 네이버 파라미터 '발생함'

![06_14_manual_example_04.png]({{"/assets/img/06_14_manual_example_04.png"| relative_url}})
 
▶ 각 정보 Pane 부분 
 . `페이지 발생 로그` 부분에 `페이지뷰(PV)`로그 1개가 발생하는데, 경우에 따라 `전환` 로그도 1개 발생할 수 있습니다. (상품상세페이지라면 `view_product`전환이 발생할 수 있음) (로그명의 오른쪽에 초록색 icon이 보임)
 . `네이버 광고 쿠키` 부분에 3개 혹은 4개의 쿠키를 볼 수 있습니다. (`NA_CO`, `NVADID` 2개는 항상 생성. 검색광고라면 `NA_SA` 생성, 성과형디스플레이광고라면 `NA_DA` 생성)
 . `네이버 파라미터` 부분에 NaPm 정보가 보이고, 표에서 파라미터 ci 와 tr 값이 보입니다.

![06_14_manual_example_05.png]({{"/assets/img/06_14_manual_example_05.png"| relative_url}})

다음과 같이 로그가 2개가 보일 수도 있습니다. (광고 클릭 후 랜딩된 곳이 상품상세페이지라면 로그가 2개 발생할 수 있음)
![06_14_manual_example_06.png]({{"/assets/img/06_14_manual_example_06.png"| relative_url}})


(4-2) 비정상적인 경우 다음과 같은 경우의 일부가 나타날 수 있습니다.

사이트ID가 0건이거나, 값이 unknown이거나, 2건 이상인 경우
 . 기본적으로 사이트ID 는 1개가 적절하며, 혹시 네이버애널리틱스를 함께 사용하시는 경우라면 2개까지는 가능합니다. 
 . 사이트ID가 0건이면 스크립트 설치가 안되어 있는 경우일 수 있습니다.
 . 사이트ID가 2건 일 수는 있으나 (네이버애널리틱스 스크립트를 광고 전환 스크립트와 함께 설치한 경우) 2건 이상 이면 의도된 것인지 살펴볼 필요가 있습니다.
 . 사이트ID가 3건 이상인 정상적인 경우는 거의 발견되지 않습니다. 스크립트가 올바로 설치되었는지 확인이 필요합니다.

로그발생이 0건이거나, 3건 이상인 경우
 . 로그발생이 0건인 경우의 예상 문제 원인
 . (예상 원인) 스크립트의 주요내용이 빠졌음. 스크립트에 오타가 있음. 페이지가 열려도 스크립트가 실행되지 않는 곳에 위치하고 있음. 이러한 문제가 있는지 살펴보고 해결이 필요합니다.
 . 로그발생이 3건 이상인 경우의 예상 문제 원인
 . (예상 원인) 스크립트가 loop 문 등으로 인하여 여러번 실행이 되고 있음. 스크립트가 중복으로 여러개가 설치되어 있음 (예: 다른 페이지를 import했는데, 그 페이지에도 광고 전환 스크립트가 있는 경우)

로그명 오른쪽에 icon이 노란색이거나, 붉은색인 경우
 . 로그의 상세 내용 중 일부에 문제가 발생한 경우입니다.
 . 표에 나타난 로그의 오른쪽 끝을 보시면 '상세보기'버튼이 있습니다. 이 버튼을 클릭하셔서 나오는 화면에서 각 항목별 주의/오류 판단결과가 표시되니, 살펴보시기 바랍니다.
 (주의/오류가 나타나는 곳은 스크립트에서 변수로 dynamic하게 값을 설정해주는 경우가 대부분입니다. 변수가 올바르게 설정이 되었는지 / data type은 맞는지 살펴봐주시기 바랍니다.)

광고를 클릭했음에도 불구하고, 네이버 광고 쿠키가 0건이거나, 2건 이하거나, 5건 이상인 경우
 . 클릭한 광고가 사실은 광고가 아닌 경우
 . 클릭한 광고가 광고 전환 추적 신청(혹은 프리미엄로그분석) 이 안되어 있는 비즈채널에 연결된 광고인 경우
 . 광고 클릭후 네이버 매체에서 광고 Link_URL에 붙여주는 NaPm파라미터가, 사이트에서 redirection되면서 사라진 경우 혹은 redirection되면서 디코딩/인코딩이 된 경우

광고를 클릭해서 사이트에 들어갔음에도 불구하고, 네이버 파라미터 `미발생` 인 경우
 . 광고 클릭후 네이버 매체에서 광고 Link_URL에 붙여주는 NaPm파라미터가, 사이트에서 redirection되면서 사라진 경우

비정상적인 경우 예시
 . 사이트ID가 3개로 비정상적으로 많은 경우 (보통 1개)
 . 발생로그가 비정상적으로 많은 경우 (보통 1개 혹은 페이지뷰(PV) + 전환로그 까지 해서 총 2개)
 . 네이버 광고 쿠키가 비정상적으로 적은 경우 (보통 2개, 3개, 4개, 혹은 0개임, 1개인 경우는 발생하지 않음)

![06_14_manual_example_07.png]({{"/assets/img/06_14_manual_example_07.png"| relative_url}})


(5) (전자상거래 사이트라면) 장바구니담기, 구매완료 등의 프로세스를 진행함
(혹은 사이트를 이용하면서 전환이벤트가 심어진 페이지 이동 혹은 전환이벤트를 발생시키는 버튼을 클릭함)

(5-1) 정상적인 경우 `어시스턴트`앱에는 다음같이 표시됩니다.
(구매완료페이지에서 구매완료(purchase)전환 로그가 발생하는 경우의 예시)

사이트ID 1개
로그 2건 (페이지 발생 로그 페이지뷰(PV) 1건, 전환(purchase) 1건), 로그명 오른쪽에 icon은 녹색 체크 icon
네이버 광고 쿠키: 3건(`NA_SA`, `NVADID`, `NA_CO`) (혹은 `NA_DA` 까지 포함하여 4건)
네이버 파라미터: 미발생 (통상적으로 구매완료페이지를 광고 랜딩으로 삼지는 않으므로, 구매완료 페이지 에서는 `미발생`인 경우가 대부분)

▶정상적인 경우 예시
![06_14_manual_example_08.png]({{"/assets/img/06_14_manual_example_08.png"| relative_url}})

(5-2) 비 정상적인 경우 혹은 오류가 있는 경우라면 다음과 같은 현상 중 일부가 발생할 수 있습니다.

사이트 ID가 0건이거나, 2건 이상인 경우 
로그발생이 0건이거나, 3건 이상인 경우
네이버 광고 쿠키가 0건이거나, 2건 이하거나, 5건 이상인 경우
페이지 발생 로그 부분에서 로그명 오른쪽에 icon이 노란색이거나, 붉은색 icon 인 경우
=> 위 (4-2)를 참고하여 문제의 원인 파악을 해주세요.

▶ 비정상적인 경우 예시
![06_14_manual_example_09.png]({{"/assets/img/06_14_manual_example_09.png"| relative_url}})
...
### FAQ목록
 - [Q1. 사이트에 네이버 광고 전환 추적 스크립트와 네이버 애널리틱스 스크립트 두개를 삽입하면 어떻게 보이나요?](#q1) anchor 만 <br>
 - [Q2. 전환로그가 발생하고 다른 페이지로 바로 이동하는 페이지의 경우 어시스턴트에서는 어떻게 보여지나요?](#q2) anchor 만 <br>
